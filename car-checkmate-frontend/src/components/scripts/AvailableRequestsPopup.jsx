  ...
  const fetchCarData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/inspections/upcoming-unclaimed-mechanic"
      );
  ...
  const handleSubmit = async (requestId) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:3000/inspections/accept-inspection-mechanic/${requestId}`,
        {
        ...
        <>
          ...
          <div className="popup-request-inspection">
            <div className="popup-content-inspection">
              <span
                className="close"
                onClick={() => setShowAvailableRequestsPopup(false)}
              >
                &times;
              </span>
              ...
              <div className="pagination">
                {Array.from(
                  { length: Math.ceil(data.length / requestsPerPage) },
                  (_, index) => (
