...
async function createInspection(req, res) {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Inspection record created successfully.", inspection });
  } catch (error) {
...
async function acceptInspection(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Inspection request accepted successfully." });
  } catch (error) {
...
async function denyInspection(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Inspection request denied successfully." });
  } catch (error) {
...
async function getPastInspectionsBuyer(req, res) {
  try {
    ...
    const pastInspections = await Inspection.find({
      buyer_id: buyerId,
      ...
      .exec();
    const inspectionsWithCarDetails = await Promise.all(
      pastInspections.map(async (inspection) => {
        let carDetails = await Car.findById(inspection.car_id);
        if (!carDetails) {
          // If car details are empty, return an object with a message
          return { ...inspection.toObject() };
...
async function getUpcomingInspectionsSeller(req, res) {
  try {
    ...
    const upcomingInspections = await Inspection.find({
      ...
      $or: [{ sellerAcceptedInspectionRequest: true }],
    })
      .sort({ inspectionDate: 1 })
      .exec();
...
async function acceptInspectionMechanic(req, res) {
  try {
    ...
    if (existingInspections.length > 0) {
      ...
      return res
        .status(400)
        .json({ error: "Cannot accept inspection. Scheduling conflict." });
    }
...
async function getAcceptedInspectionsMechanic(req, res) {
  try {
    ...
    const sortedInspections = await Inspection.find({
      mechanic_id: mechanicId,
      inspectionDate: { $gte: currentDate },
...
async function changeInspectionStatus(req, res) {
  try {
    ...
    if (inspection.mechanic_id.toString() !== userId) {
      return res
        .status(403)
        .json({ error: "You are not authorized to update this inspection." });
    }
    ...
    if (rwcCheckFile) {
      ...
      if (!validateFileType(rwcCheckFile)) {
        return res.status(400).json({
          error: "Invalid file type. Only PDF or image files are allowed.",
        });
      }
