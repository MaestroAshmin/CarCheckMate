...
//       !req.files ||
//       !req.files.carPhotos ||
//       !Array.isArray(req.files.carPhotos)
//     ) {
//       return res
//         .status(400)
...
//       return res
//         .status(400)
//         .json({ error: "Please upload between 8 to 20 car photos" });
//     }
...
const uploadCarData = async (req, res) => {
  ...
  try {
    ...
    // Save car photos to a separate folder
    const carPhotos = req.body.map((photoBlob, index) => {
      // Generate a unique file name by adding the current date and time
      const currentDate = new Date().toISOString().replace(/:/g, "-"); // Replace colons with hyphens to make it a valid file name
      const fileName = `${currentDate}_photo_${index + 1}.png`;
      // Save photo to file system
      const filePath = path.join(uploadDirectory, fileName);
      fs.writeFileSync(filePath, Buffer.from(photoBlob.replace(/^data:image\/\w+;base64,/, ""), 'base64'));
      return filePath; // Return the file path or any other necessary data
    });
