...
const uploadCarData = async (req, res) => {
  ...
  try {
    ...
    const { seller_id, make, model, streetName, suburb, postcode, state, color, price, odometer, transmission, year, engineType, fuelType, bodyType, registrationNo } = req.body;
     ...
     if (existingCar) {
       return res.status(400).json({ status: false, error: "A car with the same registration number already exists for this seller" });
     }
    ...
    if (user && !user.sellerVerified) {
      return res.status(400).json({ status: false, error: "Seller is not Verified" });
    }
    // Check if carPhotos are uploaded
    if (!req.files || Object.keys(req.files).length === 0) {
      return res.status(400).json({ status: false, error: "No car photos uploaded or invalid format" });
    }
    ...
    if (numCarPhotos < 8 || numCarPhotos > 20) {
      return res.status(400).json({ status: false, error: "Number of car photos must be between 8 and 20" });
    }
